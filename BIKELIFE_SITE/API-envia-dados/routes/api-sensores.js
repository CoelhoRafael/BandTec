...
router.get("/sendData", (request, response) => {
	...
	if (env == "dev") {
		...
		instrucaoSql = `INSERT into leitura (velocidade, momento, fkbike)
		values (${parseInt(temperature)}, '${agora()}', 1);`;
	} else {

		// Na variável abaixo, coloque o Insert que será executado no SQL Server
		// salvo exceções, é igual a Workbench

		instrucaoSql = `INSERT into dbo.leitura (velocidade, momento, fkbike)
